...
public class QuestionAndRecord : MonoBehaviour
{
    ...
    [SerializeField] private GameObject imageR;
    ...
    float[,] allAnsTime = new float[27, 8];  // 回答時間を各エリア，各回答数毎に記録（エリア０の１回目，２回目，エリア１の１回目，２回目，… の順）
    ...
    void Update()
    {
        if(questionLevel1List1.Count == 0 && questionLevel1List2.Count == 0)
        {
            questionLevel1List1 = new List<int>();
            questionLevel1List2 = new List<int>();
            characterLevel1List = new List<string>();
    ...
    void Question()
    {
        if(questionLevel1List1.Count == 0 && questionLevel1List2.Count > 0)
        {
            selectedQuestion = questionLevel1List1[UnityEngine.Random.Range(0, questionLevel1List2.Count)];
    ...
    void Disappearance()
    {
        ...
        /*if(allAnsTime[selectedQuestion - 1, ope_state*2] == 5.0f)
        {
            allAnsTime[selectedQuestion - 1, ope_state*2] = 4.0f;
        }
        else if(allAnsTime[selectedQuestion - 1, ope_state*2] < 5.0f)
        {
            allAnsTime[selectedQuestion - 1, ope_state*2 + 1] = 4.0f;
            AnsTime[selectedQuestion - 1, ope_state] = (allAnsTime[selectedQuestion - 1, ope_state*2] + allAnsTime[selectedQuestion - 1, ope_state*2 + 1]) / 2; 
        }*/
    ...
    public void LeftAnswer()
    {
        ...
        else
        {
            if(charaNow == "L")
            {
                ...
                /*if(allAnsTime[selectedQuestion - 1, ope_state*2] == 5.0f)
                {
                    allAnsTime[selectedQuestion - 1, ope_state*2] = timeAnswer;
                }
                else if(allAnsTime[selectedQuestion - 1, ope_state*2] < 5.0f)
                {
                    allAnsTime[selectedQuestion - 1, ope_state*2 + 1] = timeAnswer;
                    AnsTime[selectedQuestion - 1, ope_state] = (allAnsTime[selectedQuestion - 1, ope_state*2] + allAnsTime[selectedQuestion - 1, ope_state*2 + 1]) / 2; 
                }*/
